...
- `typesense-api.ts` — `fetchTypesenseWordSuggestions(prefix, lang, limit?, signal?)` + `isTypesenseConfigured()`: typo-tolerant as-you-type suggestions from a [Typesense](https://typesense.org) collection per language (`words_<code>`), queried directly from the app with a **search-only** key. Opt-in via `EXPO_PUBLIC_TYPESENSE_HOST` + `EXPO_PUBLIC_TYPESENSE_SEARCH_KEY` — both unset means `fetchWordSuggestions` behaves exactly as it did before. Plain `fetch`, not the official `typesense` npm client (that one ships axios; nothing else in this app uses an HTTP library). Seeded by `scripts/seed-typesense-words.mjs`. → [docs/typesense.md](docs/typesense.md).
...
[Typesense](https://github.com/typesense/typesense)'s typo-tolerant search, now backing the **word suggestion autocomplete** (opt-in via two env vars; unset = previous Datamuse/wiktapi behaviour). Book search and the community word wall are still candidates, not built. → **[docs/typesense.md](docs/typesense.md)**.
